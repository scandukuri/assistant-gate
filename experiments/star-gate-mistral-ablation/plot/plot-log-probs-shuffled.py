...
#     # Plot each category with a larger marker using plot()
    ...
    with open('star-2-mistral-ablation-qa-experimental-shuffled_means.json', 'w') as f:
        json.dump(experimental_means, f)
    with open('star-2-mistral-ablation-qa-experimental-shuffled_sems.json', 'w') as f:
        json.dump(experimental_sems, f)
    with open('star-2-mistral-ablation-qa-experimental-shuffled_topk_means.json', 'w') as f:
        json.dump(experimental_topk_means, f)
    with open('star-2-mistral-ablation-qa-experimental-shuffled_topk_sems.json', 'w') as f:
...
# Show the plot
    ...
    plt.savefig(f'{FIGURES_PATH}/{VERSION_2_MISTRAL_ABLATION}/{VERSION_2_MISTRAL_ABLATION}-log_probs_{args.split.name}-shuffled_top-{args.k}.png')
