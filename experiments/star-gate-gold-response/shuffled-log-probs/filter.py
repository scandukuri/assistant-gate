...
def main(args: DictConfig) -> None:
    ...
    with open(f"{LOGPROBS_PATH}/{VERSION_1_ESFT}/{args.condition.name}/{args.qa_model.shortname}/{args.split.name}-shuffled.json", 'r') as f:
    ...
    with open(f"{LOGPROBS_PATH}/{VERSION_1_ESFT}/{args.condition.name}/{args.qa_model.shortname}/{args.split.name}-shuffled_top-k-{args.k}.json", 'w') as f:
        json.dump(filtered_logprobs, f)
    with open(f"{LOGPROBS_PATH}/{VERSION_1_ESFT}/{args.condition.name}/{args.qa_model.shortname}/{args.split.name}-shuffled_top-k-{args.k}_indices.json", 'w') as f:
    ...
    if args.condition.name == 'qa-experimental':
        ...
        with open(f'{SIMULATION_PATH}/{VERSION_1_ESFT}/{args.qa_model.shortname}/{args.split.name}-shuffled_top-k-{args.k}.json', 'w') as f:
