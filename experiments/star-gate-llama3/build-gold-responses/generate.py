...
def main(args: DictConfig) -> None:
    ...
    if not os.path.exists(f"{GOLD_PATH}/{LLAMA_VERSION}"):
        os.makedirs(f"{GOLD_PATH}/{LLAMA_VERSION}")
    ...
    for j, persona in enumerate(personas):
        ...
        for batch_index, prompt_batch in enumerate(prompt_batches):
            ...
            if is_openai:
                ...
                # Adjust for batch processing
                system_prompt = SYS_PROMPTS[args.SYS_PROMPT_IDX]
                ...
                responses = [sanitize_response(response) for response in responses]
        ...
        with open(f"{GOLD_PATH}/{LLAMA_VERSION}/{args.split.name}.json", 'w') as f:
