...
def main(args: DictConfig) -> None:
    ...
    pulled_conversations = json.load(open(f"{SIMULATION_PATH}/{VERSION_1_MISTRAL_ABLATION}/{args.qa_model.shortname}/{args.split.name}.json", 'r'))
    ...
    if args.turn.number != args.MAX_TURNS:
        ...
        with open(f"{SIMULATION_PATH}/{VERSION_1_MISTRAL_ABLATION}/{args.qa_model.shortname}/{args.split.name}.json", 'w') as f:
            json.dump(unfinished, f)
        with open(f"{SIMULATION_PATH}/{VERSION_1_MISTRAL_ABLATION}/{args.qa_model.shortname}/{args.split.name}_turn-{args.turn.number}.json", 'w') as f:
