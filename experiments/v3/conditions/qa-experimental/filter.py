...
def main(args: DictConfig) -> None:
    ...
    for i, key in enumerate(list(logprobs.keys())):
        ...
        max_indices = np.argpartition(logprobs[key], -k)[-k:]
        ...
        filtered_logprobs[key] = [logprobs[key][i] for i in max_indices_sorted]
        filtered_conversations[key] = [conversations[key][i] for i in max_indices_sorted]


    with open(LOGPROBS_DIR[:-5] + f'_top-k-{k}.json', 'w') as f:
        json.dump(filtered_logprobs, f)
    with open(LOGPROBS_DIR[:-5] + f'_top-k-{k}_indices.json', 'w') as f:
        json.dump(filtered_indices, f)
    with open(CONVERSATIONS_DIR[:-5] + f'_top-k-{k}_conversations.json', 'w') as f:
