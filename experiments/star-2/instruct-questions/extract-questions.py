...
def main(args: DictConfig) -> None:
    ...
    if not os.path.exists(f'{PROMPT_PATH}/{VERSION_2}'):
        os.makedirs(f'{PROMPT_PATH}/{VERSION_2}')
    
    with open(f'{PROMPT_PATH}/{VERSION_2}/A.json', 'w') as f:
        json.dump(A_public_initial_turns, f)
    
    with open(f'{PROMPT_PATH}/{VERSION_2}/B.json', 'w') as f:
        json.dump(B_public_initial_turns, f)
    
    with open(f'{PROMPT_PATH}/{VERSION_2}/test.json', 'w') as f:
