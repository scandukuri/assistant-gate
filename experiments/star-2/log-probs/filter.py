...
def main(args: DictConfig) -> None:
    ...
    with open(f"{LOGPROBS_PATH}/{VERSION_2}/{args.condition.name}/{args.qa_model.shortname}/{args.split.name}.json", 'r') as f:
    ...
    with open(f"{LOGPROBS_PATH}/{VERSION_2}/{args.condition.name}/{args.qa_model.shortname}/{args.split.name}_top-k-{args.k}.json", 'w') as f:
        json.dump(filtered_logprobs, f)
    with open(f"{LOGPROBS_PATH}/{VERSION_2}/{args.condition.name}/{args.qa_model.shortname}/{args.split.name}_top-k-{args.k}_indices.json", 'w') as f:
        json.dump(filtered_indices, f)
    # THIS LAST ONE IS WRONG, fix later
    if args.condition.name == 'qa-experimental':
        if not os.path.exists(f"{SIMULATION_PATH}/{VERSION_2}/{args.qa_model.shortname}"):
            os.makedirs(f"{SIMULATION_PATH}/{VERSION_2}/{args.qa_model.shortname}")
        with open(f'{SIMULATION_PATH}/{VERSION_2}/{args.qa_model.shortname}/{args.split.name}_top-k-{args.k}.json', 'w') as f:
