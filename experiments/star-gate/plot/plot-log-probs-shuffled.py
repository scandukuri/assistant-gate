...
#     # Plot each category with a larger marker using plot()
    ...
    # with open('star-2-bsft-pos_control_1_means.json', 'w') as f:
    #     json.dump(pos_control_1_means, f)
    # with open('star-2-bsft-pos_control_1_sems.json', 'w') as f:
    #     json.dump(pos_control_1_sems, f)
    # with open('star-2-bsft-pos_control_1_topk_means.json', 'w') as f:
    #     json.dump(pos_control_1_topk_means, f)
    # with open('star-2-bsft-pos_control_1_topk_sems.json', 'w') as f:
    #     json.dump(pos_control_1_topk_sems, f)
    # with open('star-2-bsft-pos_control_2_means.json', 'w') as f:
    #     json.dump(pos_control_2_means, f)
    # with open('star-2-bsft-pos_control_2_sems.json', 'w') as f:
    #     json.dump(pos_control_2_sems, f)
    # with open('star-2-bsft-pos_control_2_topk_means.json', 'w') as f:
    #     json.dump(pos_control_2_topk_means, f)
    # with open('star-2-bsft-pos_control_2_topk_sems.json', 'w') as f:
    #     json.dump(pos_control_2_topk_sems, f)
    with open('star-2-bsft-qa-experimental-shuffled_means.json', 'w') as f:
        json.dump(experimental_means, f)
    with open('star-2-bsft-qa-experimental-shuffled_sems.json', 'w') as f:
        json.dump(experimental_sems, f)
    with open('star-2-bsft-qa-experimental-shuffled_topk_means.json', 'w') as f:
        json.dump(experimental_topk_means, f)
    with open('star-2-bsft-qa-experimental-shuffled_topk_sems.json', 'w') as f:
        json.dump(experimental_topk_sems, f)
    # with open('star-2-bsft-neg-control_means.json', 'w') as f:
    #     json.dump(neg_control_means, f)
    # with open('star-2-bsft-neg-control_sems.json', 'w') as f:
...
# Show the plot
    if not os.path.exists(f'{FIGURES_PATH}/{VERSION_2_BSFT}'):
        os.makedirs(f'{FIGURES_PATH}/{VERSION_2_BSFT}')
    # the file title is on purpose, the redundancy of VERSION_2_BSFT helps identify the experiment when we copy over outside of the regular working node (i.e. to send in Slack etc.)
    plt.savefig(f'{FIGURES_PATH}/{VERSION_2_BSFT}/{VERSION_2_BSFT}-log_probs_{args.split.name}-shuffled_top-{args.k}.png')
