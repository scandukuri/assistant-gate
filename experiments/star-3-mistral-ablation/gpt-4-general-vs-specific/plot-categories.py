...
def main(args: DictConfig) -> None:
    ...
    with open(f'{PROMPT_PATH}/{VERSION_3_MISTRAL_ABLATION}/{args.split.name}.json', "r") as f:
        prompts = json.load(f)
        prompts = [s.strip() for s in prompts]
    # Load personas
    with open(f'{PERSONAS_PATH}/{VERSION_3_MISTRAL_ABLATION}/{args.split.name}.json', "r") as f:
        personas = json.load(f)
    with open(f"{SPECIFICITY_PATH}/{VERSION_3_MISTRAL_ABLATION}/{args.split.name}_ratings.json", "r") as f:
        ratings = json.load(f)
    ...
    with open('star-3-mistral-ablation-qa-experimental-GPT4-RATED-general_means.json', 'w') as f:
        json.dump(experimental_general_means, f)
    with open('star-3-mistral-ablation-qa-experimental-GPT4-RATED-specific_means.json', 'w') as f:
        json.dump(experimental_specific_means, f)
    with open('star-3-mistral-ablation-qa-experimental-GPT4-RATED-general_topk_means.json', 'w') as f:
        json.dump(experimental_topk_general_means, f)
    with open('star-3-mistral-ablation-qa-experimental-GPT4-RATED-specific_topk_means.json', 'w') as f:
        json.dump(experimental_topk_specific_means, f)
    with open('star-3-mistral-ablation-qa-experimental-GPT4-RATED-general_sems.json', 'w') as f:
        json.dump(experimental_general_sems, f)
    with open('star-3-mistral-ablation-qa-experimental-GPT4-RATED-specific_sems.json', 'w') as f:
        json.dump(experimental_specific_sems, f)
    with open('star-3-mistral-ablation-qa-experimental-GPT4-RATED-general_topk_sems.json', 'w') as f:
        json.dump(experimental_topk_general_sems, f)
    with open('star-3-mistral-ablation-qa-experimental-GPT4-RATED-specific_topk_sems.json', 'w') as f:
    ...
    plt.tight_layout()
    # Show the plot
    if not os.path.exists(f'{FIGURES_PATH}/{VERSION_3_MISTRAL_ABLATION}'):
        os.makedirs(f'{FIGURES_PATH}/{VERSION_3_MISTRAL_ABLATION}')
    # the file title is on purpose, the redundancy of VERSION_3_MISTRAL_ABLATION helps identify the experiment when we copy over outside of the regular working node (i.e. to send in Slack etc.)
    plt.savefig(f'{FIGURES_PATH}/{VERSION_3_MISTRAL_ABLATION}/{VERSION_3_MISTRAL_ABLATION}-log_probs_gpt-4-general-vs-specific-{args.split.name}_top-{args.k}.png')
