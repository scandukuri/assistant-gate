...
def main(args: DictConfig) -> None:
    ...
    with open(f"{LOGPROBS_PATH}/{VERSION}/{args.condition.name}/{args.qa_model.shortname}/m0_{args.split.name}.json", 'r') as f:
    ...
    with open(f"{LOGPROBS_PATH}/{VERSION}/{args.condition.name}/{args.qa_model.shortname}/m0_{args.split.name}_top-k-{args.k}.json", 'w') as f:
        json.dump(filtered_logprobs, f)
    with open(f"{LOGPROBS_PATH}/{VERSION}/{args.condition.name}/{args.qa_model.shortname}/m0_{args.split.name}_top-k-{args.k}_indices.json", 'w') as f:
    ...
    if args.condition.name == 'qa-experimental':
        ...
        with open(f'{SIMULATION_PATH}/{VERSION}/{args.qa_model.shortname}/m0_{args.split.name}_top-k-{args.k}.json', 'w') as f:
