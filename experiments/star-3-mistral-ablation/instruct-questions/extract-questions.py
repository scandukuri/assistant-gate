...
def main(args: DictConfig) -> None:
    ...
    if not os.path.exists(f'{PROMPT_PATH}/{VERSION_3_MISTRAL_ABLATION}'):
        os.makedirs(f'{PROMPT_PATH}/{VERSION_3_MISTRAL_ABLATION}')
    
    with open(f'{PROMPT_PATH}/{VERSION_3_MISTRAL_ABLATION}/A.json', 'w') as f:
        json.dump(A_public_initial_turns, f)
    
    with open(f'{PROMPT_PATH}/{VERSION_3_MISTRAL_ABLATION}/B.json', 'w') as f:
        json.dump(B_public_initial_turns, f)
    
    with open(f'{PROMPT_PATH}/{VERSION_3_MISTRAL_ABLATION}/test.json', 'w') as f:
