...
with open(M0C_NEG_CONTROL_DIR, 'r') as f:
    neg_control = json.load(f)

with open(M0C_POS_CONTROL_DIR, 'r') as f:
    pos_control = json.load(f)

with open(M0C_TOPK_POS_CONTROL_DIR, 'r') as f:
    pos_control_topk = json.load(f)
    
with open(M0C_EXPERIMENTAL_DIR, 'r') as f:
    experimental = json.load(f)

with open(M0C_TOPK_EXPERIMENTAL_DIR, 'r') as f:
    experimental_topk = json.load(f)


with open(M1C_NEG_CONTROL_DIR, 'r') as f:
    m1_neg_control = json.load(f)

with open(M1C_POS_CONTROL_DIR, 'r') as f:
    m1_pos_control = json.load(f)

with open(M1C_TOPK_POS_CONTROL_DIR, 'r') as f:
    m1_pos_control_topk = json.load(f)
    
with open(M1C_EXPERIMENTAL_DIR, 'r') as f:
    m1_experimental = json.load(f)

with open(M1C_TOPK_EXPERIMENTAL_DIR, 'r') as f:
...
log_probabilities = {
    'pos-control': [pos_control_mean, m1_pos_control_mean],
    'neg-control': [neg_control_mean, m1_neg_control_mean],
    'qa-experimental': [experimental_mean, m1_experimental_mean],
...
plt.title(f'Average Log-Probability of Shuffled Desired Responses [n = {sum([len(lst) for key, lst in pos_control.items()])} simulations]')
...
# Show the plot
plt.savefig(f'figures/plot_all_confusion_top-{k}.png')
