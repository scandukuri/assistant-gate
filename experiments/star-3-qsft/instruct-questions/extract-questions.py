...
def main(args: DictConfig) -> None:
    ...
    if not os.path.exists(f'{PROMPT_PATH}/{VERSION_3_QSFT}'):
        os.makedirs(f'{PROMPT_PATH}/{VERSION_3_QSFT}')
    
    with open(f'{PROMPT_PATH}/{VERSION_3_QSFT}/A.json', 'w') as f:
        json.dump(A_public_initial_turns, f)
    
    with open(f'{PROMPT_PATH}/{VERSION_3_QSFT}/B.json', 'w') as f:
        json.dump(B_public_initial_turns, f)
    
    with open(f'{PROMPT_PATH}/{VERSION_3_QSFT}/test.json', 'w') as f:
