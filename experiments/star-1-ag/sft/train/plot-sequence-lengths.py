...
def main(args: DictConfig) -> None:
    ...
    targets = json.load(open(f"{SFT_DATA_PATH}/{VERSION_AG}/{args.model.shortname}/m0_{args.split.name}.json", 'r'))
    ...
    plt.title(f'Sequence Lengths for m0_{VERSION_AG} Split {args.model.shortname} Simulated Conversations on {args.split.name} split')
    plt.xlabel('# Tokens')
    ...
    plt.savefig(f'seq-length-plots/{VERSION_AG} {args.model.shortname}-m0_{args.split.name}-sequence-lengths.png')
