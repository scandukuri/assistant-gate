...
def main(args: DictConfig) -> None:
    ...
    with open(f"{GOLD_PATH}/{VERSION_2}/{args.qa_model.shortname}/{args.split.name}.json", 'r') as f:
    ...
    if not os.path.exists(f"{SFT_DATA_PATH}/{VERSION_2}/{args.iteration.shortname}"):
        os.makedirs(f"{SFT_DATA_PATH}/{VERSION_2}/{args.iteration.shortname}")
    with open(f"{SFT_DATA_PATH}/{VERSION_2}/{args.iteration.shortname}/{args.split.name}.json", 'w') as f:
        json.dump(targets, f)
