...
def main(args: DictConfig) -> None:
    ...
    for i in range(1, args.MAX_TURNS + 1):
        qa_responses_1.append(json.load(open(f'{WINRATE_PATH}/{VERSION_AG}/{args.qa_model.shortname}/{args.split.name}_turn-{i}_qa_responses.json', 'r')))
    
    qa_responses_2 = []
    for i in range(1, args.MAX_TURNS + 1):
        qa_responses_2.append(json.load(open(f'{WINRATE_PATH}/{VERSION_AG}/{args.qa_model_2.shortname}/{args.split.name}_turn-{i}_qa_responses.json', 'r')))
