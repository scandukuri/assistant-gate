...
def sanitize_response(
    ...
    # Define the headers you want to remove
    headers = ["assistant", "user", "system"]
    for header in headers:
        response = response.replace(f"<|start_header_id|>{header}<|end_header_id|>", "")
        response = response.replace(f"<|start_header_id|>{header}", "")
        response = response.replace(f"{header}<|end_header_id|>", "")
    return response.strip()


def flip_roles(c):
    return [{'role': 'assistant' if item['role'] == 'user' else 'user', 'content': item['content']} for item in c]


def filter_keys(conversations, persona_id):
    return [key for key in conversations.keys() if key.strip().endswith(f'persona-{persona_id}')]


def get_prompt_keys(keys):
    return [int(key[key.find('prompt-') + len('prompt-') : key.find('persona')].strip()) for key in keys]

def get_persona_keys(keys):
    return [int(key[key.find('persona-') + len('persona-'):].strip()) for key in keys]