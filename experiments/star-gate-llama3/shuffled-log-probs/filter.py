...
def main(args: DictConfig) -> None:
    ...
    # Load conversations
    with open(f'{SIMULATION_PATH}/{LLAMA_VERSION}/{args.qa_model.shortname}/{args.split.name}-shuffled.json', 'r') as f:
        conversations = json.load(f)
    
    # Load corresponding logprobs
    with open(f"{LOGPROBS_PATH}/{LLAMA_VERSION}/{args.condition.name}/{args.qa_model.shortname}/{args.split.name}-shuffled.json", 'r') as f:
    ...
    with open(f"{LOGPROBS_PATH}/{LLAMA_VERSION}/{args.condition.name}/{args.qa_model.shortname}/{args.split.name}-shuffled_top-k-{args.k}.json", 'w') as f:
        json.dump(filtered_logprobs, f)
    with open(f"{LOGPROBS_PATH}/{LLAMA_VERSION}/{args.condition.name}/{args.qa_model.shortname}/{args.split.name}-shuffled_top-k-{args.k}_indices.json", 'w') as f:
    ...
    if args.condition.name == 'qa-experimental':
        ...
        with open(f'{SIMULATION_PATH}/{LLAMA_VERSION}/{args.qa_model.shortname}/{args.split.name}-shuffled_top-k-{args.k}.json', 'w') as f:
