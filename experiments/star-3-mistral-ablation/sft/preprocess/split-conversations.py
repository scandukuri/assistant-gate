...
def main(args: DictConfig) -> None:
    ...
    with open(f"{SIMULATION_PATH}/{VERSION_3_MISTRAL_ABLATION}/{args.qa_model.shortname}/{args.split.name}_top-k-{args.k}.json", 'r') as f:
    ...
    if not os.path.exists(f"{SFT_DATA_PATH}/{VERSION_3_MISTRAL_ABLATION}/{args.qa_model.shortname}"):
        os.makedirs(f"{SFT_DATA_PATH}/{VERSION_3_MISTRAL_ABLATION}/{args.qa_model.shortname}")
    with open(f"{SFT_DATA_PATH}/{VERSION_3_MISTRAL_ABLATION}/{args.qa_model.shortname}/{args.split.name}.json", 'w') as f:
        json.dump(targets, f)
