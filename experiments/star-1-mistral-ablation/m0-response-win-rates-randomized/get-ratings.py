...
def main(args: DictConfig) -> None:
    ...
    if not os.path.exists(f'{WINRATE_PATH}/{VERSION_1_MISTRAL_ABLATION}/{args.qa_model.shortname}_{args.qa_model_2.shortname}'):
        os.makedirs(f'{WINRATE_PATH}/{VERSION_1_MISTRAL_ABLATION}/{args.qa_model.shortname}_{args.qa_model_2.shortname}')
    ...
    for i in range(1, args.MAX_TURNS + 1):
        qa_responses_1.append(json.load(open(f'{WINRATE_PATH}/{VERSION_1_MISTRAL_ABLATION}/{args.qa_model.shortname}/{args.split.name}_turn-{i}_m0-responses.json', 'r')))
    
    qa_responses_2 = []
    for i in range(1, args.MAX_TURNS + 1):
        qa_responses_2.append(json.load(open(f'{WINRATE_PATH}/{VERSION_1_MISTRAL_ABLATION}/{args.qa_model_2.shortname}/{args.split.name}_turn-{i}_m0-responses.json', 'r')))
    ...
    for t_num, group in enumerate([turns_1, turns_2, turns_3]):
        ...
        with open(f'{WINRATE_PATH}/{VERSION_1_MISTRAL_ABLATION}/{args.qa_model.shortname}_{args.qa_model_2.shortname}/{args.split.name}_turn-{t_num + 1}_m0-win-rates-randomized.json', 'w') as f:
