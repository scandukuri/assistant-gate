        ...
        for i, prompt in enumerate(prompts):
            ...
            if is_vllm:
                for lst in [first, second, third]:
                    ...
                    prompts=[[{"role" : "user", "content" : f"My name is {names[j].strip()}. Here is some information about me:\n\n{persona.strip()}\n\n{prompts[i].strip()}"}] + conversation[1:] for conversation in lst]
                    final_turns = [jsonlist[-1]['content'].strip() + '\n\n' + prompt.strip() for jsonlist in prompts]
                    prompts = [jsonlist[:-1] + [final_turns[curr]] for curr, jsonlist in enumerate(prompts)]
                    prompts = [tokenizer.apply_chat_template(jsonlist, tokenize=False) for jsonlist in prompts]
                    
                    answers = [jsonlist + [{"role": "assistant", "content": gold_responses[f'prompt-{i} persona-{j}'][0].strip()}] for jsonlist in prompts]
