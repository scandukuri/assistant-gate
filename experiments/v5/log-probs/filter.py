...
def main(args: DictConfig) -> None:
    ...
    # Load conversations
    with open(f'/sailhome/andukuri/research_projects/assistant-gate/experiments/v5/simulate-conversations-efficient/simulated-conversations/{args.qa_model.shortname}_{args.split.name}.json', 'r') as f:
        conversations = json.load(f)
    
    
    # Load corresponding logprobs
    with open(f'{args.condition.name}/{args.qa_model.shortname}_{args.split.name}.json', 'r') as f:
    ...
    with open(f"{args.condition.name}/{args.qa_model.shortname}_{args.split.name}_top-k-{args.k}.json", 'w') as f:
        json.dump(filtered_logprobs, f)
    with open(f"{args.condition.name}/{args.qa_model.shortname}_{args.split.name}_top-k-{args.k}_indices.json", 'w') as f:
        json.dump(filtered_indices, f)
    # THIS LAST ONE IS WRONG, fix later
    if args.condition.name == 'qa-experimental':
        with open(f'/sailhome/andukuri/research_projects/assistant-gate/experiments/v5/simulate-conversations-efficient/simulated-conversations/{args.qa_model.shortname}_{args.split.name}_top-k-{args.k}.json', 'w') as f:
