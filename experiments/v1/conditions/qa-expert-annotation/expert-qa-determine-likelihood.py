...
def main(args: DictConfig) -> None:
    ...
    for key in conversations.keys():
        conversation = conversations[key][0]
        i = int(key[key.find('prompt-') + 7])
        j = int(key[key.find('persona-') + 8])
        if is_vllm:
            if is_mistral:
                s, e = conversation.find(EOS_TOKEN) + 7, conversation.rfind(EOS_TOKEN) + 7
                log_probs = model.batch_log_probs(
                    prompts=[f"{BOS_TOKEN}{B_INST}My name is {names[j]}. {prompts[i]}{E_INST}{conversation[s:e]}\n\nA: "], 
                    answers=[f"{BOS_TOKEN}{B_INST}My name is {names[j]}. {prompts[i]}{E_INST}{conversation[s:e]}\n\nA: {gold_responses[f'prompt-{i} persona-{j}'][0]}"])
                final_log_probs[f'prompt-{i} persona-{j}'].extend(log_probs.tolist())

    
    with open(f'log-probs/expertqa_model-{args.model.name}.json', 'w') as f:
