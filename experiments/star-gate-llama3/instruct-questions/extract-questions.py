...
def main(args: DictConfig) -> None:
    ...
    if not os.path.exists(f'{PROMPT_PATH}/{LLAMA_VERSION}'):
        os.makedirs(f'{PROMPT_PATH}/{LLAMA_VERSION}')
    
    with open(f'{PROMPT_PATH}/{LLAMA_VERSION}/A.json', 'w') as f:
        json.dump(A_public_initial_turns, f)
    
    with open(f'{PROMPT_PATH}/{LLAMA_VERSION}/B.json', 'w') as f:
        json.dump(B_public_initial_turns, f)
    
    with open(f'{PROMPT_PATH}/{LLAMA_VERSION}/test.json', 'w') as f:
