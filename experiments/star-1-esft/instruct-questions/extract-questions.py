...
def main(args: DictConfig) -> None:
    ...
    if not os.path.exists(f'{PROMPT_PATH}/{VERSION_1_ESFT}'):
        os.makedirs(f'{PROMPT_PATH}/{VERSION_1_ESFT}')
    
    with open(f'{PROMPT_PATH}/{VERSION_1_ESFT}/A.json', 'w') as f:
        json.dump(A_public_initial_turns, f)
    
    with open(f'{PROMPT_PATH}/{VERSION_1_ESFT}/B.json', 'w') as f:
        json.dump(B_public_initial_turns, f)
    
    with open(f'{PROMPT_PATH}/{VERSION_1_ESFT}/test.json', 'w') as f:
